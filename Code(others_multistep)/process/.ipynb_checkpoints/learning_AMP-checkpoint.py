...
def Test_multistep(model,dataloader,metrics_fn):
    model.eval()
    n=len(dataloader)
    predicts=[]
    ...
    with _torch.inference_mode():
        for step in range(_PARAM.NUM_STEPs):
            ...
            for data in dataloader:
                ...
                target=target.to(_PARAM.DEVICE)
                temp.append(predict)
            temp=_torch.concatenate(temp,0)
            
                
                
                # print(f'step-{step}: ',_Format_Metrics(metrics_fn(predict.where(predict>0,0),target)))
