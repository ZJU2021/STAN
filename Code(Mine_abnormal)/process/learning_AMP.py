...
def Test(model,dataloader,metrics_fn,epoch,save=False):
    model.eval()
    n=len(dataloader)
    metrics=[]
    ...
    with _torch.inference_mode():
        for data in dataloader:
            ...
            predict=model([x.to(_PARAM.DEVICE) for x in input])
            target=target.permute(0,2,3,1).to(_PARAM.DEVICE)
            predict_=predict.where(predict>0,0)
            metrics.append(metrics_fn(predict_,target))

            predicts.append(predict_)
            targets.append(target)
    
    if save:
        if not _os.path.exists('temp'): _os.mkdir('temp')
        predict_merged = _torch.concatenate(predicts).detach().cpu().numpy()
        target_merged = _torch.concatenate(targets).detach().cpu().numpy()
        _numpy.save(f'temp/predict_merged_{epoch}.npy',predict_merged)
        _numpy.save(f'temp/target_merged_{epoch}.npy',target_merged)
...
def Loop(model,optimizer,scheduler,scaler,dataset,loss_fn,metrics_fn,
    ...
    for epoch in range(start_epoch,1+num_epochs):
        ...
        metrics={'train':Train(model,optimizer,scaler,dataloader['train'],loss_fn,metrics_fn,k),
                 'validate':Test(model,dataloader['validate'],metrics_fn,epoch),
                 'test':Test(model,dataloader['test'],metrics_fn,epoch,save=True)}
        scheduler.step()
